...
class MLXSigLIPEmbedder:
    ...
    def __init__(self, model_name: str, batch_size: int = 16, dim: int = EMBED_DIM):
    ...
    def embed_texts(self, texts: list[str]) -> np.ndarray:
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            inputs = self.processor(
                text=batch, padding="max_length", max_length=64, return_tensors="mlx"
            )
            embeds = self.model.get_text_features(input_ids=inputs["input_ids"])
            out.append(self._to_numpy(embeds))
        out_normalized = l2_normalize(np.vstack(out))
        if out_normalized.shape[1] != self.dim:
            raise ValueError(f"Expected text embedding dimension {self.dim}, got {out_normalized.shape[1]}")
    ...
    def embed_images(self, images: list[Image.Image]) -> np.ndarray:
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            embeds = self.model.get_image_features(pixel_values=inputs["pixel_values"])
            out.append(self._to_numpy(embeds))
        out_normalized = l2_normalize(np.vstack(out))
        if out_normalized.shape[1] != self.dim:
            raise ValueError(f"Expected image embedding dimension {self.dim}, got {out_normalized.shape[1]}")
        return out_normalized
...
class MLXTextEmbedder:
    ...
    def __init__(self, model_name: str = DEFAULT_TEXT_MODEL, batch_size: int = 16, dim: int = 768):
        from mlx_embeddings import load
