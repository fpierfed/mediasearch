...
def embed_dim_for(model: str) -> int:
    try:
        ...
        raise ValueError(f"Unknown model '{model}'. Known models: {known}") from None


EMBED_DIM = embed_dim_for(DEFAULT_MODEL)  # 1152; kept for backward-compatible imports
DEFAULT_INDEX_PATH = Path.home() / ".medsearch" / "index"
...
class Config:
    ...
    batch_size: int = 16
    top_k: int = 20
    fts_score_k: float = 10.0  # BM25 half-saturation constant for score normalisation
