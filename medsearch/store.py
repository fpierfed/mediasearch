...
def _normalize_scores(rows: list[dict], fts: bool = False, fts_k: float = 10.0) -> None:
...
class Store:
    def __init__(self, index_path: str | Path, dim: int = EMBED_DIM, text_dim: int = 768, fts_score_k: float = 10.0) -> None:
    ...
    def _ensure_tables(self) -> None:
        ...
        if "transcripts" not in names:
            self.db.create_table("transcripts", schema=self._transcripts_schema())
            self.db.open_table("transcripts").create_fts_index("text", replace=True)
    ...
    def set_file(self, *, path: str, mtime: float, size: int, media_type: str, status: str,
                 n_vectors: int = 0, error_msg: str | None = None) -> None:
        self.files.delete(f"path = '{_esc(path)}'")
        self.files.add([{
            "path": path, "mtime": float(mtime), "size": int(size),
            "media_type": media_type, "status": status,
            "n_vectors": int(n_vectors), "error_msg": error_msg or "",
    ...
    def count_vectors(self, media_path: str) -> int:
        return int(self.emb.count_rows(filter=f"media_path = '{_esc(media_path)}'"))

    def search(self, vector: list[float] | np.ndarray, top_k: int, media_type: str | None = None) -> list[dict]:
        q = self.emb.search(np.asarray(vector, dtype=np.float32)).metric("cosine").limit(top_k)
        if media_type:
    ...
    def search_transcripts_vector(self, vector: list[float] | np.ndarray, top_k: int) -> list[dict]:
        q = self.transcripts.search(np.asarray(vector, dtype=np.float32)).metric("cosine").limit(top_k)
        results = q.to_list()
        _normalize_scores(results)
        return results
