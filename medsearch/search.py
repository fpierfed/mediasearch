...
def _group_by_media_multi(visual_rows: list[dict], audio_rows: list[dict]) -> list[dict]:
    ...
    for r in itertools.chain(visual_rows, audio_rows):
        ...
        if r["modality"] == _MOD_VISUAL:
            groups[mp]["best_visual"] = max(groups[mp]["best_visual"], r["score"])
            groups[mp]["has_visual"] = True
        else:
            groups[mp]["best_audio"] = max(groups[mp]["best_audio"], r["score"])
...
def search_text(query: str, config: Any, embedder: Any, text_embedder: Any, store: Any, top_k: int | None = None, media_type: str | None = None) -> list[dict]:
    k = top_k or config.top_k

    vec = embedder.embed_texts([query])[0]
    visual_rows = store.search(vec, top_k=k * _OVERFETCH, media_type=media_type)
    ...
    if media_type in (None, "video", "transcript"):
        ...
        audio_vec_rows = store.search_transcripts_vector(text_vec, top_k=k * _OVERFETCH)
        audio_fts_rows = store.search_transcripts_fts(query, top_k=k * _OVERFETCH)
...
def search_image(path: str | Path, config: Any, embedder: Any, store: Any, top_k: int | None = None, media_type: str | None = None) -> list[dict]:
    k = top_k or config.top_k
...
def search_clip(path: str | Path, config: Any, embedder: Any, store: Any, top_k: int | None = None) -> list[dict]:
    ...
    k = top_k or config.top_k
    try:
        frames = sample_video(Path(path), config.frame_interval, config.dedup_threshold)
    except Exception:
    ...
    agg: dict[str, dict] = {}
    for r in store.search(pooled_vec, top_k=k * _OVERFETCH, media_type="video"):
        mp = r["media_path"]
