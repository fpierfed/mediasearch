...
def _process_audio(mf: MediaFile, text_embedder: Any, audio_model: str = DEFAULT_AUDIO_MODEL) -> list[dict]:
    try:
...
def _process(mf: MediaFile, config: Config, embedder: Any) -> list[dict]:
    ...
    frames = sample_video(mf.path, config.frame_interval, config.dedup_threshold)
    if not frames:
        return []
...
def _unchanged(prev: dict, mf: MediaFile) -> bool:
    ...
    return math.isclose(prev["mtime"], mf.mtime, rel_tol=1e-9, abs_tol=1e-3) and prev["size"] == mf.size
...
def index(
    ...
    for mf in walk([Path(r) for r in roots]):
        ...
        store.set_file(
            path=key, mtime=mf.mtime, size=mf.size,
            media_type=mf.media_type, status="pending",
        )
        try:
            visual_rows = _process(mf, config, embedder)
            transcript_rows = []
            if mf.media_type == "video":
                transcript_rows = _process_audio(mf, text_embedder, config.audio_model)
            ...
            store.set_file(
                path=key, mtime=mf.mtime, size=mf.size,
                media_type=mf.media_type, status="done", n_vectors=len(visual_rows),
            )
            ...
            store.set_file(
                path=key, mtime=mf.mtime, size=mf.size,
                media_type=mf.media_type, status="error", error_msg=str(exc),
            )
        if progress:
            progress()
