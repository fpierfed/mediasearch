...
from medsearch.frames import Frame, dedup, extract_frames, sample_video


def _frame(color, ts, idx):
    return Frame(image=Image.new("RGB", (32, 32), color), timestamp=ts, frame_idx=idx)
...
def test_sample_video_dedups_two_scenes(sample_video):
    from medsearch.frames import sample_video as sample_fn
...
def test_cgimage_to_pil_non_32bpp(monkeypatch):
    ...
    from medsearch.frames import _cgimage_to_pil

    # 2x2 RGB image = 12 bytes (2 rows × 6 bpr)
    raw = bytes([255, 0, 0, 0, 255, 0, 0, 0, 255, 0, 255, 0])

    monkeypatch.setattr("medsearch.frames.CGImageGetWidth", lambda cg: 2)
    monkeypatch.setattr("medsearch.frames.CGImageGetHeight", lambda cg: 2)
    monkeypatch.setattr("medsearch.frames.CGImageGetBytesPerRow", lambda cg: 6)
    monkeypatch.setattr("medsearch.frames.CGImageGetBitsPerPixel", lambda cg: 24)
    monkeypatch.setattr("medsearch.frames.CGImageGetDataProvider", lambda cg: object())
    monkeypatch.setattr("medsearch.frames.CGDataProviderCopyData", lambda p: raw)
