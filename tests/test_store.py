...
from medsearch.store import Store, _esc
...
def test_set_and_read_manifest(tmp_path):
    ...
    s.set_file(path="/a.png", mtime=1.0, size=10, media_type="image", status="done", n_vectors=1)
    m = s.manifest()
...
def test_set_file_upserts(tmp_path):
    s = Store(tmp_path / "idx")
    s.set_file(path="/a.png", mtime=1.0, size=10, media_type="image", status="pending")
    s.set_file(path="/a.png", mtime=1.0, size=10, media_type="image", status="done", n_vectors=2)
    m = s.manifest()
...
def test_search_media_type_filter(tmp_path):
    ...
    s.add_embeddings([_row("/v.mp4", _unit(1), media_type="video", ts=2.0, idx=1)])
...
def test_stats(tmp_path):
    s = Store(tmp_path / "idx")
    s.set_file(path="/a.png", mtime=1.0, size=1, media_type="image", status="done", n_vectors=1)
    s.set_file(path="/b.mp4", mtime=1.0, size=1, media_type="video", status="error", error_msg="boom")
    s.add_embeddings([_row("/a.png", _unit(1))])
    st = s.stats()
    assert st["files"] == 2 and st["done"] == 1 and st["error"] == 1 and st["vectors"] == 1
...
def test_reopen_existing_index_does_not_recreate(tmp_path):
    ...
    path = tmp_path / "idx"
    s1 = Store(path)
    s1.set_file(path="/a.png", mtime=1.0, size=10, media_type="image", status="done", n_vectors=1)
...
def test_transcripts_operations(tmp_path):
    ...
    # test vector search
    v_hits = s.search_transcripts_vector(np.ones(768, dtype=np.float32), top_k=1)
    assert len(v_hits) == 1
