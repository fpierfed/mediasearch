...
from medsearch.cli import app

runner = CliRunner()


def test_index_then_status(tmp_path, make_image, monkeypatch):
    monkeypatch.setenv("MEDSEARCH_FAKE_EMBEDDER", "1")
    lib = tmp_path / "lib"
    lib.mkdir()
...
def test_search_json_output(tmp_path, make_image, monkeypatch):
    ...
    r = runner.invoke(app, ["search", "a photo", "--index-path", str(idx), "--json"])
    assert r.exit_code == 0, r.output
...
def test_similar_image(tmp_path, make_image, monkeypatch):
    ...
    r = runner.invoke(
        app, ["similar-image", str(lib / "a.png"), "--index-path", str(idx), "--json"]
    )
    assert r.exit_code == 0, r.output
    data = json.loads(r.output)
...
def test_invalid_type_is_rejected(tmp_path, make_image, monkeypatch):
    monkeypatch.setenv("MEDSEARCH_FAKE_EMBEDDER", "1")
    lib = tmp_path / "lib"; lib.mkdir(); make_image(lib / "a.png")
    idx = tmp_path / "idx"
    runner.invoke(app, ["index", str(lib), "--index-path", str(idx)])
    r = runner.invoke(app, ["search", "q", "--index-path", str(idx), "--type", "bogus"])
    assert r.exit_code != 0
    assert "bogus" in r.output or "image" in r.output  # message names the bad value or valid choices
...
def test_model_load_failure_is_friendly(tmp_path, make_image, monkeypatch):
    ...
    monkeypatch.delenv("MEDSEARCH_FAKE_EMBEDDER", raising=False)
    import medsearch.embedder as emb
    ...
    assert ("model" in r.output.lower()) or ("apple silicon" in r.output.lower())
...
def test_bad_model_is_rejected(tmp_path, monkeypatch):
    monkeypatch.setenv("MEDSEARCH_FAKE_EMBEDDER", "1")
    idx = tmp_path / "idx"
    r = runner.invoke(app, ["status", "--index-path", str(idx), "--model", "nope"])
    assert r.exit_code != 0
    assert "nope" in r.output or "Unknown model" in r.output


def test_dim_mismatch_without_reindex_errors(tmp_path, make_image, monkeypatch):
    monkeypatch.setenv("MEDSEARCH_FAKE_EMBEDDER", "1")
    lib = tmp_path / "lib"; lib.mkdir(); make_image(lib / "a.png")
    idx = tmp_path / "idx"
    # build a 1152-d index with the default model
    assert runner.invoke(app, ["index", str(lib), "--index-path", str(idx)]).exit_code == 0
    # now query/index with the 768-d base model, no reindex -> friendly error
    r = runner.invoke(app, ["search", "q", "--index-path", str(idx), "--model", BASE_MODEL])
    assert r.exit_code != 0
...
def test_dim_mismatch_with_reindex_rebuilds(tmp_path, make_image, monkeypatch):
    ...
    r = runner.invoke(app, ["index", str(lib), "--index-path", str(idx), "--model", BASE_MODEL, "--reindex"])
    assert r.exit_code == 0, r.output
    # now searching with the base model works (dims match) and returns the image
    r2 = runner.invoke(app, ["search", "q", "--index-path", str(idx), "--model", BASE_MODEL, "--json"])
...
def test_similar_clip(tmp_path, make_image, sample_video, monkeypatch):
    ...
    r = runner.invoke(
        app, ["similar-clip", str(lib / "clip.mp4"), "--index-path", str(idx), "--json"]
    )
    assert r.exit_code == 0, r.output
    data = json.loads(r.output)
    assert isinstance(data, list)
...
def test_open_flag(tmp_path, make_image, monkeypatch):
    ...
    r = runner.invoke(app, ["search", "a photo", "--index-path", str(idx), "--open"])
    assert r.exit_code == 0, r.output
...
def test_text_dim_mismatch_errors(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MEDSEARCH_FAKE_EMBEDDER", "1")
    import typer
    from medsearch.cli import _guard_text_dim
    from medsearch.store import Store
...
def test_build_embedder_text_model_success(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MEDSEARCH_FAKE_EMBEDDER", raising=False)
    import medsearch.embedder as emb
    from medsearch.cli import _build_embedder
    from medsearch.config import Config, DEFAULT_TEXT_MODEL
