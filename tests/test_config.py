...
from medsearch.config import Config, EMBED_DIM, classify_ext, IMAGE_EXTS, VIDEO_EXTS
...
def test_defaults():
    ...
    assert c.index_path == Path.home() / ".medsearch" / "index"
...
def test_embed_dim_property_and_resolver():
    from medsearch.config import Config, embed_dim_for
    assert Config().embed_dim == 1152
    # 768-dim (base/medium)
    assert Config(model="mlx-community/siglip2-base-patch16-384").embed_dim == 768
    assert embed_dim_for("mlx-community/siglip2-base-patch16-384") == 768
    assert Config(model="mlx-community/siglip2-base-patch16-256").embed_dim == 768
    ...
    assert Config(model="mlx-community/siglip-large-patch16-384").embed_dim == 1024
    ...
    # quantised
    assert Config(model="mlx-community/siglip-large-patch16-384-4bit").embed_dim == 1024
    assert Config(model="mlx-community/siglip2-base-patch16-224-8bit").embed_dim == 768


def test_embed_dim_unknown_model_raises():
    import pytest
    from medsearch.config import embed_dim_for
