...
def test_reindex_skips_unchanged(tmp_path, make_image):
    ...
    index(config, FakeEmbedder(), FakeEmbedder(), store, [lib])  # second run, nothing changed
    second_indexed_at = store.manifest()["%s" % (lib / "a.png")]["indexed_at"]
    assert first_indexed_at == second_indexed_at   # row was NOT rewritten -> skipped
...
def test_resume_reprocesses_pending(tmp_path, make_image):
    ...
    store.set_file(path=str(p), mtime=mf["mtime"], size=mf["size"],
...
def test_process_audio_success(tmp_path, sample_video, monkeypatch):
    import mlx_whisper
    from medsearch.config import DEFAULT_AUDIO_MODEL
    from medsearch.pipeline import _process_audio
    from medsearch.embedder import FakeEmbedder
    from medsearch.walker import MediaFile
...
def test_process_audio_empty_segments(tmp_path, sample_video, monkeypatch):
    ...
    monkeypatch.setattr(mlx_whisper, "transcribe", lambda *a, **kw: {"segments": []})
...
def test_process_no_frames_for_video(tmp_path, monkeypatch):
    ...
    from medsearch.pipeline import _process
    from medsearch.config import Config
    from medsearch.embedder import FakeEmbedder
    from medsearch.walker import MediaFile

    monkeypatch.setattr("medsearch.pipeline.sample_video", lambda *a, **kw: [])

    mf = MediaFile(path=tmp_path / "empty.mp4", mtime=0.0, size=0, media_type="video")
