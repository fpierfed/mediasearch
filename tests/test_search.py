from medsearch.config import Config
from medsearch.embedder import FakeEmbedder
from medsearch.pipeline import index
from medsearch.search import (
    format_timestamp,
    search_clip,
    search_image,
    search_text,
...
def test_search_image_returns_exact_match_first(tmp_path, make_image, sample_video):
...
def test_results_are_grouped_one_row_per_file(tmp_path, make_image, sample_video):
    config, store, lib = _library(tmp_path, make_image, sample_video)
    results = search_image(lib / "a.png", config, FakeEmbedder(), store)
    paths = [r["path"] for r in results]
    assert len(paths) == len(set(paths))  # the multi-frame clip appears at most once
...
def test_search_text_runs_and_is_structured(tmp_path, make_image, sample_video):
    config, store, lib = _library(tmp_path, make_image, sample_video)
    results = search_text("anything", config, FakeEmbedder(), FakeEmbedder(dim=768), store, top_k=2)
    assert len(results) <= 2
    assert {"rank", "score", "path", "media_type", "timestamp", "time", "modality"} <= set(results[0])
...
def test_search_text_aggregates_scores_and_modalities():
    ...
    class MockStore:
        def search(self, vec, top_k, media_type):
            return [{"media_path": "clip.mp4", "score": 0.5, "media_type": "video", "timestamp": 10.0}]
            
        def search_transcripts_vector(self, vec, top_k):
            return [{"media_path": "clip.mp4", "score": 0.4, "media_type": "transcript", "start_time": 10.0}]
            
        def search_transcripts_fts(self, query, top_k):
            return [{"media_path": "clip.mp4", "score": 0.2, "media_type": "transcript", "start_time": 12.0}]
            
    config = Config()
    store = MockStore()
    results = search_text("query", config, FakeEmbedder(), FakeEmbedder(dim=768), store, top_k=2)
...
def test_search_clip_no_frames(tmp_path, make_image, monkeypatch):
    ...
    monkeypatch.setattr("medsearch.search.sample_video", lambda *a, **kw: [])
